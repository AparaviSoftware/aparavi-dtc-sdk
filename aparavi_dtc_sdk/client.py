...
class AparaviClient:
    ...
    def validate_pipe(self, pipeline: Dict[str, Any]) -> ResultBase:
        response = self._make_request(method="POST", endpoint="/pipe/validate", json=pipeline)
    ...
    def start_task(
        ...
        response = self._make_request(method="PUT", endpoint="/task", json=pipeline, params=params)
    ...
    def send_to_webhook_with_file(self, token: str, task_type: str, file_glob: str) -> List[Dict[str, Any]]:
    ...
    def run_pipeline(
        ...
        """
        try:
            result = self.validate_pipe(pipeline)
        ...
        try:
            task_result = self.start_task(pipeline, name=task_name)
            ...
            is_webhook = pipeline.get("source", "").startswith("webhook")

            if is_webhook:
                self._log("Webhook pipeline detected. Polling until task is running...", self.COLOR_ORANGE)

                for attempt in range(max_attempts):
                    status_result = self.get_task_status(token, task_type=task_type)
                ...
                responses = self.send_to_webhook_with_file(
                    token=token,
                    task_type=task_type,
                    file_glob=file_glob
                )
                ...
                final_status = self.get_task_status(token=token, task_type=task_type)
                self._log(f"Final Task status: {final_status.data}", self.COLOR_GREEN)

                end_result = self.end_task(token=token, task_type=task_type)
